...
# ============================================================
# Case 7 | 剩余注意力 PINN | 连续训练 50000 轮主文件
#
# 说明：
# 1. 与 case6 MLP / case6 LSTM 使用同一套制造解
# 2. 连续训练 50000 轮
# 3. 输出 best_model.pt / checkpoint / statistics / 图像 / txt
# ============================================================

MODEL_TYPE = "resatt_pinn"
CASE_NAME = "case7_multifront_channel"
SEED = 42
...
# ============================================================
# Case 7 真解（与 case6 MLP / LSTM 一致）
# ============================================================

def psi_true(x, y):
    main_center = 0.14 + 0.66 * x + 0.07 * torch.sin(1.48 * math.pi * x + 0.18)
    side_center = 0.84 - 0.42 * x + 0.04 * torch.cos(1.18 * math.pi * x + 0.24)

    return (
        0.056 * torch.tanh(14.5 * (y - main_center))
        - 0.022 * torch.tanh(10.8 * (y - side_center))
        + 0.024 * torch.sin(1.18 * math.pi * x + 0.10) * torch.sin(0.98 * math.pi * y + 0.08)
        + 0.026 * torch.exp(-28.0 * ((x - 0.22) ** 2 + (y - 0.26) ** 2))
        - 0.021 * torch.exp(-24.0 * ((x - 0.76) ** 2 + (y - 0.76) ** 2))
        + 0.018 * torch.exp(-20.0 * ((x - 0.54) ** 2 + (y - 0.46) ** 2))
    )


def p_true_func(x, y):
    return (
        0.108 * torch.cos(1.00 * math.pi * x + 0.14) * torch.sin(0.90 * math.pi * y + 0.16)
        + 0.102 * torch.exp(-31.0 * ((x - 0.80) ** 2 + (y - 0.30) ** 2))
        - 0.086 * torch.exp(-26.0 * ((x - 0.28) ** 2 + (y - 0.68) ** 2))
        + 0.030 * torch.exp(-18.0 * ((x - 0.58) ** 2 + (y - 0.52) ** 2))
    )


def T_true_func(x, y):
    front_T = 0.58 - 0.18 * torch.exp(-12.0 * (x - 0.50) ** 2) + 0.07 * torch.sin(1.42 * math.pi * x + 0.08)

    return (
        0.84
        + 0.13 * x - 0.07 * y
        + 0.24 * torch.tanh(9.8 * (y - front_T))
        + 0.060 * torch.exp(-26.0 * ((x - 0.34) ** 2 + (y - 0.66) ** 2))
        + 0.045 * torch.exp(-22.0 * ((x - 0.78) ** 2 + (y - 0.20) ** 2))
        + 0.028 * torch.sin(0.95 * math.pi * x + 0.05) * torch.sin(0.82 * math.pi * y + 0.10)
    )


def phi_true_func(x, y):
    front_phi = 0.46 - 0.20 * torch.exp(-11.0 * (x - 0.52) ** 2) + 0.09 * torch.sin(1.62 * math.pi * x + 0.15)

    return (
        0.43 * torch.tanh(11.0 * (y - front_phi))
        + 0.038 * torch.exp(-29.0 * ((x - 0.66) ** 2 + (y - 0.32) ** 2))
        - 0.032 * torch.exp(-24.0 * ((x - 0.18) ** 2 + (y - 0.60) ** 2))
        + 0.032 * torch.sin(1.28 * math.pi * x + 0.04) * torch.sin(1.02 * math.pi * y + 0.06)
        + 0.024 * x + 0.010 * y
    )
...
class ResAttentionPINN(nn.Module):
    def __init__(self, in_dim=2, out_dim=5, width=120, num_blocks=6):
        super().__init__()
...
def main():
    print("=" * 72)
    print("Case 7 | 剩余注意力 PINN | 连续训练 50000 轮")
